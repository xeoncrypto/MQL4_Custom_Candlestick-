...
#property strict
...
#ifndef WIN32_DISABLE
   #ifdef __MQL4__
   
      #include "..\MT4BackCompatability\WinAPI\winapi.mqh"
   ...
   #import "user32.dll"
     ...
     // int      PostMessageA(int hWnd,int Msg,int wParam,int lParam);
      ...
      //int      keybd_event(int bVk, int bScan,int dwFlags,int dwExtrainfo); 
      void     mouse_event(int dwFlags,int dx,int dy,int dwData,int dwExtraInfo);
      
      
      int  PostMessageW(int hWnd,int Msg,int wParam,int lParam); 
      uint RegisterWindowMessageA(const string lpString);
     ...
     // int      SetWindowTextA(int hWnd,char &lpString[]);
      HANDLE      GetWindowTextW(HANDLE hWnd, string& lpString);
    //  int      GetWindowTextA(int hWnd,char &lpString[],int nMaxCount);
      HANDLE      GetWindowTextW(HANDLE hWnd, string& lpString, int nMaxCount);
  ...
  //    int      GetActiveWindow();
     ...
     // int      MessageBoxW(int hWnd,string lpText,string lpCaption,int uType);
      ...
      int      GetClassNameW(HANDLE hWnd, string& lpClassname, int nMaxCount);
